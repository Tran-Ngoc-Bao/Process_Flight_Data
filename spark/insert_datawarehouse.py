...
def solution():
    ...
    df_tmp_ref = df_ref.withColumn("hash_key_origin", sha2(df_ref.Origin, 256)).withColumn("hash_key_origin_state", sha2(df_ref.OriginState, 256)).withColumn("hash_key_dest", sha2(df_ref.Dest, 256)).withColumn("hash_key_dest_state", sha2(df_ref.DestState, 256))
    df_tmp_ref.createOrReplaceTempView("view_tmp_ref")

    spark.sql("insert into table hub_origin select hash_key_origin as hash_key, FlightDate as load_date, 'reference data' as record_source from view_tmp_ref")
    spark.sql("insert into table hub_origin_state select hash_key_origin_state as hash_key, FlightDate as load_date, 'reference data' as record_source from view_tmp_ref")
    spark.sql("insert into table hub_dest select hash_key_dest as hash_key, FlightDate as load_date, 'reference data' as record_source from view_tmp_ref")
    spark.sql("insert into table hub_dest_state select hash_key_dest_state as hash_key, FlightDate as load_date, 'reference data' as record_source from view_tmp_ref")

    spark.sql("""insert into table sat_origin
              select hash_key_origin as hash_key, FlightDate as load_date, 'reference data' as record_source,
    ...
    spark.sql("""insert into table sat_origin_state
              select hash_key_origin_state as hash_key, FlightDate as load_date, 'reference data' as record_source,
    ...
    spark.sql("""insert into table sat_dest
              select hash_key_dest as hash_key, FlightDate as load_date, 'reference data' as record_source,
              Dest as dest, DestCityName as dest_city_name, FlightDate as flight_date
              from view_tmp_ref""")
    spark.sql("""insert into table sat_dest_state
              select hash_key_dest_state as hash_key, FlightDate as load_date, 'reference data' as record_source,
    ...
    df_tmp_tran = df_tran.withColumn("hash_key", sha2(df_tran.id, 256))
