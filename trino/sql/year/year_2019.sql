...
create table if not exists hive.datasets_year.marketing_airline_network_2019
with (format = 'parquet')
as select
Year, Quarter, Month,
...
create table if not exists hive.datasets_year.flights_across_days_of_the_week_2019
with (format = 'parquet')
as select
...
create table if not exists hive.datasets_year.flight_origins_2019
with (format = 'parquet')
as select
Year, Quarter, Month,
...
create table if not exists hive.datasets_year.flights_cancellations_by_day_of_the_week_2019
with (format = 'parquet')
as select
Year, Quarter, Month, DayOfWeek,
...
create table if not exists hive.datasets_year.total_cancellations_and_flights_per_carrier_2019
with (format = 'parquet')
as select
...
create table if not exists hive.datasets_year.day_of_month_2019_new
with (format = 'parquet')
as select
...
create table if not exists hive.datasets_year.mean_delay_by_aircraft_carrier_2019
with (format = 'parquet')
as select
