...
create table if not exists hive.datasets_month.marketing_airline_network_2018_11
with (format = 'parquet')
as select
Year, Quarter, Month,
...
create table if not exists hive.datasets_month.flights_across_days_of_the_week_2018_11
with (format = 'parquet')
as select
...
create table if not exists hive.datasets_month.flight_origins_2018_11
with (format = 'parquet')
as select
Year, Quarter, Month,
...
create table if not exists hive.datasets_month.flights_cancellations_by_day_of_the_week_2018_11
with (format = 'parquet')
as select
Year, Quarter, Month, DayOfWeek,
...
create table if not exists hive.datasets_month.total_cancellations_and_flights_per_carrier_2018_11
with (format = 'parquet')
as select
...
(select Year, Quarter, Month, count(1) total_flights, count_if(Cancelled = 1) total_cancellations from hive.processed_data.flight_2018 where Month = 11 group by Year, Quarter, Month) b
where s.Year = b.Year and s.Quarter = b.Quarter and s.Month = b.Month and s.Month = 11
group by s.Year, s.Quarter, s.Month, s.Marketing_Airline_Network, b.total_flights, b.total_cancellations;



create table if not exists hive.datasets_month.day_of_month_2018_11
with (format = 'parquet')
as select
...
from hive.processed_data.flight_2018 s,
(select Year, Quarter, Month, count(1) all, count_if(Cancelled = 1) cancelled from hive.processed_data.flight_2018 where Month = 11 group by Year, Quarter, Month) b
where s.Year = b.Year and s.Quarter = b.Quarter and s.Month = b.Month and s.Month = 11
group by s.Year, s.Quarter, s.Month, s.DayofMonth, b.all, b.cancelled;



create table if not exists hive.datasets_month.mean_delay_by_aircraft_carrier_2018_11
with (format = 'parquet')
as select
