...
create table if not exists hive.datasets_quarter.marketing_airline_network_2021_1
with (format = 'parquet')
as select
Year, Quarter, Month,
...
create table if not exists hive.datasets_quarter.flights_across_days_of_the_week_2021_1
with (format = 'parquet')
as select
...
create table if not exists hive.datasets_quarter.flight_origins_2021_1
with (format = 'parquet')
as select
Year, Quarter, Month,
...
create table if not exists hive.datasets_quarter.flights_cancellations_by_day_of_the_week_2021_1
with (format = 'parquet')
as select
Year, Quarter, Month, DayOfWeek,
...
create table if not exists hive.datasets_quarter.total_cancellations_and_flights_per_carrier_2021_1
with (format = 'parquet')
as select
...
(select Year, Quarter, Month, count(1) total_flights, count_if(Cancelled = 1) total_cancellations from hive.processed_data.flight_2021 where Quarter = 1 group by Year, Quarter, Month) b
...
create table if not exists hive.datasets_quarter.day_of_month_2021_1
with (format = 'parquet')
as select
...
from hive.processed_data.flight_2021 s,
(select Year, Quarter, Month, count(1) all, count_if(Cancelled = 1) cancelled from hive.processed_data.flight_2021 where Quarter = 1 group by Year, Quarter, Month) b
where s.Year = b.Year and s.Quarter = b.Quarter and s.Month = b.Month and s.Quarter = 1
group by s.Year, s.Quarter, s.Month, s.DayofMonth, b.all, b.cancelled;



create table if not exists hive.datasets_quarter.mean_delay_by_aircraft_carrier_2021_1
with (format = 'parquet')
as select
